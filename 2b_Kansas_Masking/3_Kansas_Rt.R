...
data.model <- df.in %>% mutate(unit=ncounty,
...
Rt_est.AME <- data.frame(type=c("point estimate","lower bound","upper bound"),
  coef=c(Rt_est.coef, lower.bound, upper.bound), AME=NA_real_, AME.adj1=NA_real_, AME.adj2=NA_real_)

for (k in seq_len(nrow(Rt_est.AME))) {
  coef_i <- as.numeric(Rt_est.AME$coef[k])
  ...
  Rt_est.AME$AME[k] <- mean(sim.out$AME)
  Rt_est.AME$AME.adj1[k] <- mean(sim.out$AME.adj1)
  Rt_est.AME$AME.adj2[k] <- mean(sim.out$AME.adj2)
}
print(paste0("AME: ", format(round(Rt_est.AME$AME[1],1), nsmall=1), " with CI: (",
             format(round(Rt_est.AME$AME[2],1), nsmall=1), ", ",
             format(round(Rt_est.AME$AME[3],1), nsmall=1), ")"))
