rm(list=ls())
here::i_am("1b_Summarize/1a_SIR_summ.R")
source("./global_options.R")
source("./1a_Scripts/0_Format_Table.R")

p_out <- readRDS("4_Output/SIR_base_case_R=1.rds") %>%
...
ggarrange(p1, p2, ncol = 1, nrow = 2, heights = c(0.88, 1))
# ggsave("./4_Output/Figure2.png", width = 10, height = 6)
##############################################################################################################################
# Make kable
format.tbl(power.df1, bias.original1, bias.AME1) %>% kable(format = "latex")
